...
const PurchaseReturns = () => {
  ...
  return (
    <Layout>
      <div className="mb-6">
        <h2 className="text-xl font-bold text-gray-800">Purchase Returns &amp; Debit Notes</h2>
        <p className="text-xs text-gray-400 mt-0.5">Track items returned to suppliers and stock deductions ({pagination.total} records)</p>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6">
        <input
          type="text"
          placeholder="Search by Return No, PO No, Supplier..."
          value={search}
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-gray-50 text-gray-400 uppercase tracking-wider font-semibold border-b border-gray-100">
              <tr>
                ...
                <th className="py-3.5 px-4">Original PO</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr><td colSpan={7} className="text-center py-16 text-gray-400"><HiOutlineReply className="mx-auto mb-1 block text-2xl" aria-hidden="true" />No purchase returns recorded.</td></tr>
              ) : returns.map((r) => (
                <tr key={r._id} className="hover:bg-gray-50/70 transition">
                  ...
                  <td className="py-3 px-4 font-mono text-blue-600 font-semibold">{r.purchaseNumber}</td>
                  <td className="py-3 px-4 font-semibold text-gray-800">{r.supplierSnapshot?.name || "Supplier"}</td>
